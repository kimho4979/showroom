				...
				<div class="info_box">
					...
					<div class="slide_box web">
						<c:forEach items="${fixFileList}" var="file" varStatus="status">
							<span class="img_size">
								<img src="${pageContext.request.contextPath}/uploads/fixSmallAuc/N/${file.thumbStreFileNm}" alt="사진_${result.pumName}">
							</span>
						</c:forEach>
					</div>
						...
						<table>
							...
							<tbody>
								...
								<tr>
									...
									<td class="tc" colspan="3"><p class="txt_01 dib">${result.chulName}(${fn:substring(result.chulCode,0,4)} - ${fn:substring(result.chulCode,4,8)})</p><!-- <a href="#layerSample3" class="btn_type_01 btn-popup fix ml10" onclick="fn_popfucus();">문자</a> --></td>
...
/* function fn_delete(fixDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixNAuc/fixAdmDelProc.do?fixDealSeq="+fixDealSeq;
} */
...
function fn_ban(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmBan.json",
...
function fn_bidAdmin(bidSeq, fixDealSeq, bidAdminYn){
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmBidAdmin.json",
        success : function(data){
             alert("정삭적으로 처리되었습니다.");
             location.reload(true);
        }
	...
	if(bidCancelText != null && bidCancelText != ""){
		if(confirm("입찰 취소하시겠습니까?")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmBidCancel.json",
		        success : function(data){
		             alert("정삭적으로 처리되었습니다.");
		             location.reload(true);
		        }
...
function fn_bidUpdate(bidSeq, fixDealSeq){
	...
	if(confirm("입찰 수정하시겠습니까?")){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmBidUpdate.json",
	        success : function(data){
	             alert("정삭적으로 수정되었습니다.");
	             location.reload(true);
	        }
...
function fn_smsMms(code , codeType){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/sendSmsMms.json",
        success : function(data){
             if(data.result == "1"){
            	alert("문자가 발송되었습니다.");
            	$('.dim-layer').fadeOut();
