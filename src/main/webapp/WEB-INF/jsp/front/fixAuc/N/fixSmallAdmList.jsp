...
function fn_view(fixDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmView.do?fixDealSeq="+fixDealSeq;
}

function fn_reg(){
	window.location.href = "${pageContext.request.contextPath}/front/fixNAuc/fixAdmReg.do";
}
...
function fn_more(){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/fixAdmList.json",
...
function fn_order(index){
	...
	$.ajax({
		...
		type: "POST",
		url: "${pageContext.request.contextPath}/front/fixNAuc/tableOrder.json"
	})
	/* 정렬 유지 추가 4 (E) */
}
...
function fn_apply(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmApply.json",
	        success : function(data){
	             alert("신청된 정가수의매매 " + data.result + "건 승인처리되었습니다.");
	             // location.reload(true);
	             /* 정렬 유지 수정 1 (S) - 채성주 [ 2021.11.26 ] */
	             location.href = "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmList.do?history=true";
	             /* 정렬 유지 수정 1 (E) */
	        }
	    });
	}
...
function fn_comp(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmComp.json",
	        success : function(data){
...
function fn_appCancel(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmAppCancel.json",
	        success : function(data){
...
/*function fn_sms(){
	...
	if(confirm(html)){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmsProc.json",
	        success : function(data){
	            console.log(data);
	            alert("문자가 발송되었습니다.");
	        }
...
function getFixSmall() {
	...
	if(confirm("재상장 내역을 불러오시겠습니까?")){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmSangJang.json",
	        success : function(data){
	        	...
	        	if(data.errorCode != null) {
	        		...
	        		if(data.errorCode == '9997') {
						errorMessage = "[재상장 내역 불러오기] 화요일, 목요일, 토요일만 가능합니다.";	        			
	        		}
	        		...
	        		$.ajax({
	        	        ...
	        	        type : "POST",
	        	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmSangJangRecord.json",
	        	        success : function(data){}
	        	    });
...
function getFixSmall_del() {
	...
	if(confirm( "해당 일자("+beforeSmallUpDay +") 내역을 삭제하시겠습니까?")){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmSangJangDel.json",
	        success : function(data){
	        	...
	        	if(data.errorCode != null) {
	        		...
	        		if(data.errorCode == '9997') {
						errorMessage = "[재상장 내역 삭제] 화요일, 목요일, 토요일만 가능합니다.";	        			
	        		}
	        		if(data.errorCode == '9996') {
...
function fn_update(fixDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmUpt.do?fixDealSeq="+fixDealSeq;
}
...
function getFixSmall_excel() {
	...
	window.location.href = "${pageContext.request.contextPath}/front/fixNAuc/fixSmallListExcell.do?upDay="+smallUpDay;
}
						...
						<div class="info_box">
							...
							<div class="sub_tab">
								<ul class="devide_3">
									...
									<li><a href="${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmList.do" class="active">절화</a></li>
									</c:if>
									<!--  
							...
							<form id="frm" name="searchFrm" method="get" action="${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmList.do" >
							<!-- 정렬 유지 수정 5 (E) -->
												...
												<td class="tc">
													<c:if test="${result.fixState eq '1' or result.fixState eq '2' or result.fixState eq '3' or result.fixState eq '7'}">
														<c:if test="${empty result.vrPanDay and empty result.vrPanNo}">
															...
															<form id="frm_${result.fixDealSeq}" name="frm" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/front/fixNAuc/fixSmallAdmUptProc.do" >
