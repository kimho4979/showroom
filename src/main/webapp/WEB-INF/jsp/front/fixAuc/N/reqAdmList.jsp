...
function fn_view(reqDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixNAuc/reqAdmView.do?reqDealSeq="+reqDealSeq;
}

function fn_reg(){
	window.location.href = "${pageContext.request.contextPath}/front/fixNAuc/reqAdmReg.do";
}
...
function fn_more(){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/reqAdmList.json",
...
function fn_apply(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/reqAdmApply.json",
	        success : function(data){
	        	console.log(data);
	             alert("신청된 요청 " + data.result + "건 진행처리되었습니다.");
	             // location.reload(true);
	             /* 정렬 유지 수정 1 (S) - 채성주 [ 2021.11.26 ] */
	             location.href = "${pageContext.request.contextPath}/front/fixNAuc/reqAdmList.do?history=true";
	             /* 정렬 유지 수정 1 (E) */
	        }
	    });
	}
...
function fn_comp(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/reqAdmComp.json",
	        success : function(data){
...
function fn_appCancel(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/reqAdmAppCancel.json",
	        success : function(data){
...
/* function fn_sms(){
	...
	if(confirm(html)){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/reqSmsProc.json",
	        success : function(data){
	            console.log(data);
	            alert("문자가 발송되었습니다.");
	        }
...
function fn_ban_action() {
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/reqAdmBan.json",
        success : function(data){
						...
						<div class="info_box">
							...
							<div class="sub_tab">
								<ul class="devide_3">
									...
									<li><a href="${pageContext.request.contextPath}/front/fixNAuc/reqAdmList.do" class="active">절화</a></li>
									</c:if>
							...
							<form id="frm" name="searchFrm" method="get" action="${pageContext.request.contextPath}/front/fixNAuc/reqAdmList.do" >
							<!-- 정렬 유지 수정 6 (E) -->
								...
								<table id="fixTable">
									...
									<thead>
										<tr>
											...
											<th style="cursor: pointer;" onclick="fn_order(4);">지역</th>
											<th style="cursor: pointer;" onclick="fn_order(5);">품목명</th>
											<th style="cursor: pointer;" onclick="fn_order(6);">품종명</th>
											<th style="cursor: pointer;" onclick="fn_order(7);">중도매인명</th>
											<th style="cursor: pointer;" onclick="fn_order(8);">중도매인코드</th>
											<th style="cursor: pointer;" onclick="fn_order(9);">등급</th>
											<th style="cursor: pointer;" onclick="fn_order(10);">상자수</th>
											<th style="cursor: pointer;" onclick="fn_order(11);">속수량</th>
											<th style="cursor: pointer;" onclick="fn_order(12);">구매단가</th>
											<!-- <th>조정단가</th> -->
									...
									<tbody id="resultWebList">
										<c:forEach items="${resultList}" var="result" varStatus="status">
											<tr id="fixTr${result.reqDealSeq}">
												...
												<td class="tc" style="cursor:pointer" onclick="fn_view('${result.reqDealSeq}');"><p class="txt_01">${result.pumName}</p><c:if test="${result.twinCnt ne '0'}">/${result.twinCnt}</c:if></td>
												<td class="tc" style="cursor:pointer" onclick="fn_view('${result.reqDealSeq}');">
