				...
				<div class="info_box">
					...
					<div class="m_col_wrap">
						<div class="m_col_left">
							<!-- 테이블03(S) -->
							<form id="frm" name="frm" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/front/fixNAuc/reqBuyRegProc.do" >
								...
								<div class="table_type_03">
									<table>
										...
										<tbody>
													...
													<select id="jCode" name="jCode" onchange="getRecentList();">
														<c:forEach items="${floList}" var="floVO" varStatus="status">
															<c:if test="${floVO.bunChk eq 'N'}">
																<option value="${floVO.code}" <c:if test="${nFloLoginVO.chulCd eq  floVO.code}">selected="selected"</c:if>>${floVO.code} ( ${floVO.name} )</option>
															</c:if>
														</c:forEach>	
													</select>
													</div>
											...
											<input type="hidden" name="boxCode" value="200">
											<!--  
											<tr>
												<th class="tc">상자형</th>
												<td class="tl">
													...
													<div class="sel_type_03 w50p">
														<select id="boxCode" name="boxCode">
															<c:forEach items="${boxList}" var="boxRow">
																<option value="${boxRow.boxCode}">[ ${boxRow.boxCode} ] ${boxRow.boxName}</option>
															</c:forEach>
														</select>
													</div>
