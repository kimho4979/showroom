						...
						<table>
							...
							<tbody>
								...
								<tr>
									...
									<td class="tc"><p class="txt_01 dib">${result.chulName}(${fn:substring(result.chulCode,0,4)} - ${fn:substring(result.chulCode,4,8)})</p><a href="#layerSample3" class="btn_type_01 btn-popup fix ml10" onclick="fn_popfucus();">문자</a></td>
									<th class="tc">판매단가</th>
								...
								<tr>
									...
									<td class="tc">
										<div class="date_box">
											...
											<input type="text" class="datepicker" id="aucDate" value="${result.chulDate}">
											</c:if>
											<c:if test="${result.aucDate ne '-' and result.aucDate ne null}">
											<input type="text" class="datepicker" id="aucDate" value="${result.aucDate}">
											</c:if>
										</div>
									</td>
...
function fn_update(fixDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixNAuc/fixAdmUpt.do?fixDealSeq="+fixDealSeq;
}
...
function fn_ban(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/fixAdmBan.json",
...
function fn_apply(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/fixAdmApply.json",
        success : function(data){
...
function fn_comp(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/fixAdmComp.json",
        success : function(data){
...
function fn_appCancel(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/fixAdmAppCancel.json",
        success : function(data){
...
function fn_bidAdmin(bidSeq, fixDealSeq, bidAdminYn){
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixNAuc/fixAdmBidAdmin.json",
        success : function(data){
             alert("정삭적으로 처리되었습니다.");
             location.reload(true);
        }
	...
	if(bidCancelText != null && bidCancelText != ""){
		if(confirm("입찰 취소하시겠습니까?")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixNAuc/fixAdmBidCancel.json",
		        success : function(data){
		             alert("정삭적으로 처리되었습니다.");
		             location.reload(true);
		        }
...
function fn_bidUpdate(bidSeq, fixDealSeq){
	...
	if(confirm("입찰 수정하시겠습니까?")){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixNAuc/fixAdmBidUpdate.json",
	        success : function(data){
	             alert("정삭적으로 수정되었습니다.");
	             location.reload(true);
	        }
