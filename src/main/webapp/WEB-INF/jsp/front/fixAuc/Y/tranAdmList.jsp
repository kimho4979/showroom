...
function fn_board(){
	...
	var url = contextPath + "/front/fixYAuc/transAdmCompList.do?chulDate="+chulDate; 
	...
	if(reqAppCnt>0){
		if(confirm("요청 미완료 "+reqAppCnt+"건이 존재합니다. 그래도 마감하시겠습니까?")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixYAuc/tranMagamInsProc.json",
...
function fn_magamCancel(){
	...
	if(confirm(chulDate + " 마감취소하시겠습니까?")){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixYAuc/tranMagamDelProc.json",
	        success : function(data){
	        	if(data.errorCode == '1'){
	...
	if(checkedArray.length == 0){
		...
		if(confirm("체크된 마감자료를 전송하시겠습니까? \n 전송된 데이터는 삭제 할 수 없습니다.")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixYAuc/tranMagamComp.json",
		        success : function(data){
		        	if(data.errorCode == '1'){
						...
						<div class="info_box">
							...
							<div class="sub_tab">
								<ul class="devide_3">
									...
									<li><a href="${pageContext.request.contextPath}/front/fixYAuc/tranAdmList.do" class="active">&nbsp;&nbsp;&nbsp;난&nbsp;&nbsp;&nbsp;</a></li>
									</c:if>
							...
							<!-- 검색조건창(S) -->
							<form id="frm" name="searchFrm" method="post" action="${pageContext.request.contextPath}/front/fixYAuc/tranAdmList.do" >
						...
						<table>
							...
							<div class="title_box mt30">
								...
								<div class="fr">
									...
									<a href="${pageContext.request.contextPath}/front/fixYAuc/tranCompListExcell.do?chulDate=<c:out value="${fixInfo.chulDate}"/>" class="btn_type_01 gray ml10">마감데이터 엑셀다운로드</a>
								</div>
