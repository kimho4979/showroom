...
<!-- sub내용(S) -->
			<div class="sub_conts_in">
				...
				<div class="info_box">
					...
					<!-- 슬라이드 이미지(S) /uploads/fixAuc/Y/-->
					
					<div class="slide_box web">
						<c:forEach items="${fixFileList}" var="file" varStatus="status">
							<span class="img_size">
								<img src="${pageContext.request.contextPath}/uploads/fixSmallAuc/Y/${file.thumbStreFileNm}" alt="사진_${result.pumName}">
							</span>
						</c:forEach>
					</div>
...
/* function fn_delete(fixDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixYAuc/fixAdmDelProc.do?fixDealSeq="+fixDealSeq;
} */
...
function fn_ban(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixYAuc/fixSmallAdmBan.json",
...
function fn_bidAdmin(bidSeq, fixDealSeq, bidAdminYn){
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixYAuc/fixSmallAdmBidAdmin.json",
        success : function(data){
             alert("정삭적으로 처리되었습니다.");
             location.reload(true);
        }
	...
	if(bidCancelText != null && bidCancelText != ""){
		if(confirm("입찰 취소하시겠습니까?")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixYAuc/fixSmallAdmBidCancel.json",
		        success : function(data){
		             alert("정삭적으로 처리되었습니다.");
		             location.reload(true);
		        }
...
function fn_bidUpdate(bidSeq, fixDealSeq){
	...
	if(confirm("입찰 수정하시겠습니까?")){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixYAuc/fixSmallAdmBidUpdate.json",
	        success : function(data){
	             alert("정삭적으로 수정되었습니다.");
	             location.reload(true);
	        }
