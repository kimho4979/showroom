				...
				<div class="info_box">
					...
					<div class="m_col_wrap">
						<div class="m_col_left">
							<!-- 테이블03(S) -->
							<form id="frm" name="frm" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/front/fixYAuc/fixSellRegProc.do" >
													...
													<select id="chulCode" name="chulCode" onchange="getRecentList();">
														<c:forEach items="${floList}" var="floVO" varStatus="status">
															<c:if test="${floVO.bunChk eq 'Y'}">
																<option value="${floVO.code}" <c:if test="${yFloLoginVO.chulCd eq  floVO.code}">selected="selected"</c:if>>${fn:substring(floVO.code,0,4)} - ${fn:substring(floVO.code,4,8)} ( ${floVO.name} )</option>
															</c:if>
														</c:forEach>	
													</select>
													</div>
							...
							<div class="btn_box mt30">
								...
								<a href="${pageContext.request.contextPath}/front/fixYAuc/fixSellList.do" class="btn_type_01 list" id="listBtn">목록</a>
							</div>
						</div>
...
function fn_addSave(){
	...
	$.ajax({
        ...
        cache: false,
        timeout: 600000,
        url : "${pageContext.request.contextPath}/front/fixYAuc/fixSellRegProc.json",
