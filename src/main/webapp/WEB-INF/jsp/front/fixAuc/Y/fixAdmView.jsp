				...
				<div class="info_box">
					...
					<div class="slide_box web">
						<c:forEach items="${fixFileList}" var="file" varStatus="status">
							<span class="img_size">
								<img src="${pageContext.request.contextPath}/uploads/fixAuc/Y/${file.thumbStreFileNm}" alt="사진_${result.pumName}">
							</span>
						</c:forEach>
					</div>
...
function fn_update(fixDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixYAuc/fixAdmUpt.do?fixDealSeq="+fixDealSeq;
}
...
function fn_ban(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixYAuc/fixAdmBan.json",
...
function fn_apply(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixYAuc/fixAdmApply.json",
        success : function(data){
...
function fn_comp(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixYAuc/fixAdmComp.json",
        success : function(data){
...
function fn_appCancel(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixYAuc/fixAdmAppCancel.json",
        success : function(data){
...
function fn_bidAdmin(bidSeq, fixDealSeq, bidAdminYn){
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixYAuc/fixAdmBidAdmin.json",
        success : function(data){
             alert("정삭적으로 처리되었습니다.");
             location.reload(true);
        }
	...
	if(bidCancelText != null && bidCancelText != ""){
		if(confirm("입찰 취소하시겠습니까?")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixYAuc/fixAdmBidCancel.json",
		        success : function(data){
		             alert("정삭적으로 처리되었습니다.");
		             location.reload(true);
		        }
...
function fn_bidUpdate(bidSeq, fixDealSeq){
	...
	if(confirm("입찰 수정하시겠습니까?")){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixYAuc/fixAdmBidUpdate.json",
	        success : function(data){
	             alert("정삭적으로 수정되었습니다.");
	             location.reload(true);
	        }
