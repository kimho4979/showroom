...
function fn_view(reqDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/reqAdmView.do?reqDealSeq="+reqDealSeq;
}

function fn_reg(){
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/reqAdmReg.do";
}
...
function fn_more(){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/reqAdmList.json",
...
function fn_apply(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixCAuc/reqAdmApply.json",
	        success : function(data){
	        	console.log(data);
	             alert("신청된 요청 " + data.result + "건 진행처리되었습니다.");
	             // location.reload(true);
	             /* 정렬 유지 수정 1 (S) - 채성주 [ 2021.11.26 ] */
	             location.href = "${pageContext.request.contextPath}/front/fixCAuc/reqAdmList.do?history=true";
	             /* 정렬 유지 수정 1 (E) */
	        }
	    });
	}
...
function fn_comp(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixCAuc/reqAdmComp.json",
	        success : function(data){
...
function fn_appCancel(){
	...
	if(checkedArray.length == 0){
		...
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixCAuc/reqAdmAppCancel.json",
	        success : function(data){
...
/* function fn_sms(){
	...
	if(confirm(html)){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixCAuc/reqSmsProc.json",
	        success : function(data){
	            console.log(data);
	            alert("문자가 발송되었습니다.");
	        }
...
function fn_reqTable(){
	...
	if(chulDate == null || chulDate == ""){
		...
		window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/reqAdmTable.do?chulDate="+chulDate;
...
function fn_ban_action() {
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/reqAdmBan.json",
        success : function(data){
	...
	if(checkedArray.length == 0){
		...
		if(confirm("체크된 마감자료에 대해 운송업체[차량번호]를 변경하시겠습니까? \n ( 운송업체: "+trans+", 차량번호: "+pumKgInt+" )")) {
			...
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixCAuc/tranUpdateTransReq.json",
		        success : function(data){
		        	if(data.errorCode == '1'){
		        		/*
						...
						<div class="info_box">
							...
							<div class="sub_tab">
								<ul class="devide_3">
									...
									<li><a href="${pageContext.request.contextPath}/front/fixYAuc/reqAdmList.do">&nbsp;&nbsp;&nbsp;난&nbsp;&nbsp;&nbsp;</a></li>
									</c:if>
									<c:if test="${cFloLoginVO ne null }">
									<li><a href="${pageContext.request.contextPath}/front/fixCAuc/reqAdmList.do" class="active">관엽</a></li>
									</c:if>
								</ul>
							</div>
							<!-- 서브탭(E) -->
							...
							<form id="frm" name="searchFrm" method="get" action="${pageContext.request.contextPath}/front/fixCAuc/reqAdmList.do" >
							<!-- 정렬 유지 수정 6 (E) -->
						...
						<div class="info_box mt30">
							...
							<div class="btn-tab-new mt20 d9">
								<ul>
									...
									<li class="btn-chk-icon return"><a href="javascript:void(0);" onclick="fn_changeTrans();"><i><img src="/yfmc/img/btn-new-08.png" alt="운송업체"></i><p>운송업체</p></a></li><!--반려-->
								...
								<table id="fixTable">
									...
									<thead>
										<tr>
											...
											<th style="cursor: pointer;" onclick="fn_order(3);">지역</th>
											<th style="cursor: pointer;" onclick="fn_order(4);">품목명</th>
											<th style="cursor: pointer;" onclick="fn_order(5);">품종명</th>
											<!-- <th style="cursor: pointer;" onclick="fn_order(7);">중도매인명</th> -->
											...
											<th style="cursor: pointer;" onclick="fn_order(9);">분수량</th>
											<th style="cursor: pointer;" onclick="fn_order(10);">구매단가</th>
											<th style="cursor: pointer;" onclick="fn_order(11);">판매유형</th>
									...
									<tbody id="resultWebList">
										<c:forEach items="${resultList}" var="result" varStatus="status">
											<tr id="fixTr${result.reqDealSeq}">
												...
												<td class="tc" style="cursor:pointer" onclick="fn_view('${result.reqDealSeq}');"><p class="txt_01">${result.chulName}</p></td>
												<!-- <td class="tc" style="cursor:pointer" onclick="fn_view('${result.reqDealSeq}');"><p class="txt_01">${fn:substring(result.chulCode,0,4)} - ${fn:substring(result.chulCode,4,8)}</p></td> -->
												...
												<!-- <td class="tc" style="cursor:pointer" onclick="fn_view('${result.reqDealSeq}');"><p class="txt_01">${result.jName}</p></td> -->
												<td class="tc" style="cursor:pointer" onclick="fn_view('${result.reqDealSeq}');"><p class="txt_01">${result.jCode}</p></td>
											    ...
											    <td class="tc" style="cursor:pointer" onclick="fn_view('${result.reqDealSeq}');">
												    <p class="txt_01">${result.trans} [ <c:if test="${result.pumKg != null}">${result.pumKg}</c:if><c:if test="${result.pumKg == null}">없음</c:if> ]</p>
											    </td>
												<c:choose>
							...
							<div class="btn_box mt30">
								...
								<div class="date_box vm">
									<input type="text" class="datepicker" name="chulDate" id="chulDate" value="">
								</div>
								<a href="javascript:fn_reqTable();" class="btn_type_01 gray">중도매인 요청표<a>
							</div>
						</div>
...
<!-- 요청관리 버튼(8개)(S) -->
<div class="btn-tab-new right d9">
	<ul>
		<li class="btn-chk-icon agree"><a href="javascript:fn_apply();"><i><img src="/yfmc/img/btn-new-01.png" alt="진행"></i><p>진행</p></a></li><!--승인-->				
		<li class="btn-chk-icon a-cancel"><a href="javascript:fn_appCancel();"><i><img src="/yfmc/img/btn-new-02.png" alt="진행취소"></i><p>진행취소</p></a></li><!--승인취소-->				
		<li class="btn-chk-icon complete"><a href="javascript:fn_comp();"><i><img src="/yfmc/img/btn-new-03.png" alt="체결"></i><p>체결</p></a></li><!--완료-->				
