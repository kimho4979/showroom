...
function fn_view(fixDealSeq){
	...
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/fixBuyView.do?fixDealSeq="+fixDealSeq+"&jCode="+jCode;
}

function fn_reg(){
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/fixBuyReg.do";
}
...
function fn_more(){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/fixBuyList.json",
        success : function(data){
           ...
           if(resultList.length > 0){
        	   for(var i=0; i<resultList.length; i++ ){
					...
					html += "	<td class=\"tc td_add\"><p class=\"txt_01\">"+resultList[i].pumName+"</p></td>                                                                ";
					html += "	<input type=\"hidden\" id=\"resultPUnit"+resultList[i].fixDealSeq+"\" value=\""+resultList[i].pUnit2+"\">";
					...
					mhtml += "					<div class=\"fl\">                                                                                                                    ";
					mhtml += "						<h4 class=\"sub_tit_05\">["+resultList[i].aucDate+"] "+resultList[i].mokName+"("+resultList[i].pumName+")</h4>                                         ";
					mhtml += "					</div>                                                                                                                                ";
					mhtml += "					<div class=\"fr\">                                                                                                                    ";
					if (resultList[i].fixState == "3"){
					...
					mhtml += "							<p class=\"txt_01\">수량 : "+comma(resultList[i].sokCnt)+"분 "+comma(resultList[i].boxCnt)+"상자</p>                              ";
...
function fn_wbid(index, fixDealSeq){
	...
	var resultPUnit = $("#resultPUnit"+fixDealSeq).val();
	fn_bid(fixDealSeq, bidBoxCnt, bidSokCnt, bidUnitPrice, resultPUnit );
}
...
function fn_bid(fixDealSeq, bidBoxCnt, bidSokCnt, bidUnitPrice, resultPUnit ){
	...
	$.ajax({
		data:{
			...
			resultPUnit: resultPUnit,
			code: code
	        },
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/fixBuyBid.json",
        success : function(data){
           if(data.result == 1){
        	   alert("정상적으로 입찰되었습니다");
						...
						<div class="info_box">
							...
							<div class="sub_tab">
								<ul class="devide_3">
									...
									<li><a href="${pageContext.request.contextPath}/front/fixCAuc/fixBuyList.do" class="active">관엽</a></li>
							...
							<!-- 검색조건창(S) -->
							<form id="frm" name="searchFrm" method="post" action="${pageContext.request.contextPath}/front/fixCAuc/fixBuyList.do" >
