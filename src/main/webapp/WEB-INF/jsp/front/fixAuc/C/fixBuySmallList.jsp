...
function fn_view(fixDealSeq){
	...
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/fixBuySmallView.do?fixDealSeq="+fixDealSeq+"&jCode="+jCode;
}

/* function fn_reg(){
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/fixBuySmallReg.do";
} */
...
function fn_more(){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/fixBuySmallList.json",
        success : function(data){
           console.log("더보기페이징:",data);
           var html = "";
           var mhtml = "";
           ...
           if(resultList.length > 0){
        	   for(var i=0; i<resultList.length; i++ ){
        		   ...
        		   var twinCnt = resultList[i].twinCnt;  
           	    	...
           	    	html += "<tr>                                                        ";
					html += "<td class=\"tc td_add\" onclick=\"fn_view('"+resultList[i].fixDealSeq+"');\" style=\"cursor: pointer;\"><p class=\"txt_01\">"+resultList[i].upNo+"</p></td>                                                          ";
					html += "<td class=\"tc td_add\" onclick=\"fn_view('"+resultList[i].fixDealSeq+"');\" style=\"cursor: pointer;\"><p class=\"txt_01\">"+resultList[i].mokName+"</p></td>                                                                ";
					...
					html += "</td>";   
					if(resultList[i].hasOwnProperty("stdPrice")){
    					html += "<td class=\"tc td_add\"><p class=\"txt_01\">"+comma(resultList[i].stdPrice)+"</p></td>";
					} else {
						html += "<td class=\"tc td_add\"><p class=\"txt_01\">"+"0"+"</p></td>";
					}
					...
					html += "	</div>";
					html += "</td>";
					html += "<td class=\"tc td_add\" onclick=\"fn_view('"+resultList[i].fixDealSeq+"');\" style=\"cursor: pointer;\"><p class=\"txt_01\">"+comma(resultList[i].sokCnt)+"</p>                ";
					if(resultList[i].fixState != "4" && resultList[i].fixState != "5" && resultList[i].fixState != "6" && (resultList[i].fixState == "7" || resultList[i].bidAdminYn == "Y")){
						// html += "<div class=\"ip_type_01 w100p mt10\">";
						html += "	<input type=\"hidden\" id=\"bidSokCnt"+(rowIndex+i)+"\" value=\""+resultList[i].sokCnt+"\">";
						// html += "</div>";
					...
					html += "</td>";
					html += "<td class=\"tc td_add\" onclick=\"fn_view('"+resultList[i].fixDealSeq+"');\" style=\"cursor: pointer;\"><p class=\"txt_01\">"+comma(resultList[i].boxCnt)+"</p>                ";
					if(resultList[i].fixState != "4" && resultList[i].fixState != "5" && resultList[i].fixState != "6" && (resultList[i].fixState == "7" || resultList[i].bidAdminYn == "Y")){
						// html += "<div class=\"ip_type_01 w100p mt10\">";
						html += "	<input type=\"hidden\" id=\"bidBoxCnt"+(rowIndex+i)+"\" value=\""+resultList[i].boxCnt+"\">";
						// html += "</div>";
					...
					html += "</td>";
					html += "<td class=\"tc td_add\" onclick=\"fn_view('"+resultList[i].fixDealSeq+"');\" style=\"cursor: pointer;\"><p class=\"txt_01\">"+pumName+"</p></td>                                                                ";
					html += "<td class=\"tc td_add\" onclick=\"fn_view('"+resultList[i].fixDealSeq+"');\" style=\"cursor: pointer;\"><p class=\"txt_01\">"+resultList[i].chulLevelName+"</p></td>                                                          ";
					html += "<td class=\"tc td_add\" onclick=\"fn_view('"+resultList[i].fixDealSeq+"');\" style=\"cursor: pointer;\">                                                                                                              ";
					...
					html += "</td>                                                                                                                          ";
					html += "<td class=\"tc td_add\" onclick=\"fn_view('"+resultList[i].fixDealSeq+"');\" style=\"cursor: pointer;\"><p class=\"txt_01\">"+resultList[i].aucDate+"</p></td>                                                             ";
					html += "</tr>                                                                                                                             ";
...
function fn_bid(fixDealSeq, bidBoxCnt, bidSokCnt, bidUnitPrice ){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/fixBuySmallBid.json",
        success : function(data){
           if(data.result == 1){
        	   alert("정상적으로 입찰되었습니다");
...
function getTradeAmt(){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/getLimitAmt.json",
        success : function(data){
           console.log(data);
						...
						<div class="info_box">
							...
							<div class="sub_tab">
								<ul class="devide_3">
									...
									<li><a href="${pageContext.request.contextPath}/front/fixCAuc/fixBuySmallList.do" class="active">관엽</a></li>
									</c:if>
									<%-- <c:if test="${yFloLoginVO ne null }">
									<li><a href="${pageContext.request.contextPath}/front/fixYAuc/fixBuySmallList.do">&nbsp;&nbsp;&nbsp;난&nbsp;&nbsp;&nbsp;</a></li>
									</c:if>
									<c:if test="${cFloLoginVO ne null }">
									<li><a href="${pageContext.request.contextPath}/front/fixCAuc/fixBuySmallList.do">관엽</a></li>
									<li><a href="${pageContext.request.contextPath}/front/fixCAuc/fixBuySmallSubList.do">관엽-분갈이</a></li>
									</c:if> --%>
								</ul>
							</div>
							<!-- 서브탭(E) -->
							...
							<!-- 검색조건창(S) -->
							<form id="frm" name="searchFrm" method="post" action="${pageContext.request.contextPath}/front/fixCAuc/fixBuySmallList.do" >
											...
											<c:forEach items="${floList}" var="floVO" varStatus="status">
												<c:if test="${floVO.bunChk eq 'C'}">
													<c:if test="${code eq null}">
														<option value="${floVO.code}" <c:if test="${cFloLoginVO.chulCd eq  floVO.code}">selected="selected"</c:if>>${floVO.name} ( ${floVO.code} )</option>
													</c:if>
							...
							<div class="table_type_04 mt5 overflow_a">
								<table>
									...
									<colgroup>
										...
										<col style="width:108.54px;">
										<col style="width:83.27px;">
										<col style="width:83.27px;">
										<col style="width:83.27px;">
										<col style="width:83.27px;">
										<col style="width:83.27px;">
										<col style="width:83.27px;">
										<col style="width:98.27px;">
										<col style="width:53px;">
										<col style="width:98.54px;">
										<col style="width:83.27px;">
									</colgroup>
									<thead>
										<tr>
