...
function fn_board(){
	...
	var url = contextPath + "/front/fixCAuc/transAdmCompList.do?chulDate="+chulDate; 
	...
	if(reqAppCnt>0){
		if(confirm("요청 미완료 "+reqAppCnt+"건이 존재합니다. 그래도 마감하시겠습니까?")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixCAuc/tranMagamInsProc.json",
...
function fn_magamCancel(){
	...
	if(confirm(chulDate + " 마감취소하시겠습니까?")){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixCAuc/tranMagamDelProc.json",
	        success : function(data){
	        	if(data.errorCode == '1'){
...
function fn_tran(){
	...
	// var panType = $("input[name=panType]:checked").val();
	...
	if(checkedArray.length == 0){
		...
		if(confirm("체크된 마감자료를 전송하시겠습니까? \n 전송된 데이터는 삭제 할 수 없습니다.")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixCAuc/tranMagamComp.json",
		        success : function(data){
		        	if(data.errorCode == '1'){
	...
	if(checkedArray.length == 0){
		...
		if(confirm("체크된 마감자료에 대해 운송업체[차량번호]를 변경하시겠습니까? \n ( 운송업체: "+trans+", 차량번호: "+pumKgInt+" )")) {
			...
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixCAuc/tranMagamUpdateTrans.json",
		        success : function(data){
		        	if(data.errorCode == '1'){
		        		var upDay = data.errorVO.upDay;
		        		var upNo = data.errorVO.upNo;
						...
						<div class="info_box">
							...
							<div class="sub_tab">
								<ul class="devide_3">
									...
									<li><a href="${pageContext.request.contextPath}/front/fixNAuc/tranAdmList.do">절화</a></li>
									</c:if>
									...
									<li><a href="${pageContext.request.contextPath}/front/fixCAuc/tranAdmList.do" class="active">관엽</a></li>
									</c:if>
								</ul>
							</div>
							<!-- 서브탭(E) -->
							...
							<!-- 검색조건창(S) -->
							<form id="frm" name="searchFrm" method="post" action="${pageContext.request.contextPath}/front/fixCAuc/tranAdmList.do" >
							<input type="hidden" id="searchPanType" name="searchPanType" value="<c:out value="${paramMap.searchPanType}"/>">
						...
						<table>
							...
							<tbody>
								<tr>
									...
									<td class="tc"><p class="txt_01"><fmt:formatNumber value="${fixInfo.totBoxCnt}" pattern="#,###" />상자 (<fmt:formatNumber value="${fixInfo.totSokCnt}" pattern="#,###" />분) / <fmt:formatNumber value="${fixInfo.totTradePrice}" pattern="#,###" />원</p></td>
								</tr>
								<tr>
									<th class="tc">판매 거래량</th>
									<td class="tc"><p class="txt_01"><fmt:formatNumber value="${fixInfo.fixBoxCnt}" pattern="#,###" />상자 (<fmt:formatNumber value="${fixInfo.fixSokCnt}" pattern="#,###" />분)</p></td>
								...
								<tr>
									...
									<td class="tc"><p class="txt_01"><fmt:formatNumber value="${fixInfo.reqBoxCnt}" pattern="#,###" />상자 (<fmt:formatNumber value="${fixInfo.reqSokCnt}" pattern="#,###" />분)</p></td>
									...
									<td class="tc" colspan="3">
										<div class="sb_data ml10">
											<label class="radio mr10">
												<input type="radio" name="panType" value="03" <c:if test="${paramMap.searchPanType eq null or paramMap.searchPanType eq '03'}">checked="checked"</c:if>>
												<i class="rdo"></i>
												<em class="label-title">선취</em>
											</label>
											<label class="radio mr10">
												<input type="radio" name="panType" value="04" <c:if test="${paramMap.searchPanType eq '04'}">checked="checked"</c:if>>
												<i class="rdo"></i>
							...
							<div class="title_box mt30">
								...
								<div class="fr">
									<a href="javascript:fn_board();" class="btn_type_01 gray ml5">게시</a>
									<a href="javascript:fn_magam();" class="btn_type_01 gray ml5">마감</a>
									<a href="javascript:fn_magamCancel();" class="btn_type_01 gray ml5">마감취소</a> 
									<a href="javascript:fn_tran();" class="btn_type_01 gray ml5">체크된행 전송</a>
									<!--  
									<a href="javascript:fn_changeTrans();" class="btn_type_01 gray ml5">체크된행 운송업체 변경</a>
									-->
									<a href="${pageContext.request.contextPath}/front/fixCAuc/tranCompListExcell.do?chulDate=<c:out value="${fixInfo.chulDate}"/>" class="btn_type_01 gray ml5">마감데이터 엑셀다운로드</a>
								</div>
								...
								<table id="fixTable">
									...
									<thead>
										<tr>
											...
											<th style="cursor: pointer;" onclick="fn_order(11);">운송업체<br>[차량번호]</th>
											<th style="cursor: pointer;" onclick="fn_order(12);">전송상태</th>
										</tr>
									</thead>
									<tbody id="resultWebList">
										<c:forEach items="${resultList}" var="result" varStatus="status">
											<tr id="fixTr${result.tranSeq}">
												...
												<td class="tc" style="cursor:pointer" onclick="fn_view('${result.tranSeq}');"><p class="txt_01">${result.pumName}</p></td>
												...
												<td class="tc" style="cursor:pointer" onclick="fn_view('${result.tranSeq}');"><p class="txt_01">${result.trans} [ <c:if test="${result.pumKg != null}">${result.pumKg}</c:if><c:if test="${result.pumKg == null}">없음</c:if> ]</p></td>
												<td class="tc" style="cursor:pointer" onclick="fn_view('${result.tranSeq}');">
													<p class="txt_01">
											...
											<td>
												<div class="table_info">
													...
													<ul class="profile_box" onclick="fn_view('${result.tranSeq}');" style="cursor: pointer;">
														...
														<li>
															<p class="txt_01">낙찰분수 : <fmt:formatNumber value="${result.sokCnt}" pattern="#,###" /></p>
