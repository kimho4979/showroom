				...
				<div class="info_box">
					...
					<div class="m_col_wrap">
						<div class="m_col_left">
							<!-- 테이블03(S) -->
							<form id="frm" name="frm" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/front/fixCAuc/reqBuyUptProc.do" >
										...
										<tbody>
											...
											<tr>
												...
												<td class="tl">
													...
													<input type="hidden" id="originPUnit2" name="originPUnit2" value="${result.pUnit2}"/>
													<input type="hidden" id="pUnit2" name="pUnit2" value="${result.pUnit2}"/>
							...
							<div class="btn_box mt30">
								...
								<a href="${pageContext.request.contextPath}/front/fixCAuc/reqBuyList.do" class="btn_type_01 list">목록</a>
							</div>
						</div>
...
function getEnterJongList(){
	...
	if(searchJongName == "" || searchJongName == null){
		alert("품목명을 입력해 주세요.");
		$("#searchJongName").focus();
		return false;
	}
	...
	$.ajax({
        ...
        success : function(data){
           ...
           if(resultList.length > 1){
        	   for(var i=0; i<resultList.length; i++ ){
           	    html += "<tr onclick=\"fn_jongClick('"+resultList[i].pMokCode+"','"+resultList[i].pMokName.replace("\"","&quot;")+"','"+resultList[i].id+"','"+resultList[i].text.replace("\"","&quot;")+"','"+resultList[i].pUnit2+"')\" style=\"cursor: pointer;\">           ";
   	   			html += "	<td class=\"tc\"><a href=\"#!\" class=\"txt_01\">"+resultList[i].id+"</a></td>       ";
        	   ...
        	   $("#pumName").val(resultList[0].text);
       		   ...
       		   $("#pUnit2").val(resultList[0].pUnit2);
       		   $("#chulLevel").focus();
       		   getPreList();
           }else{
...
function fn_jongClick(mokCode, mokName, pumCode, pumName, pUnit2){
	 $("#searchMokName").val(mokName);
	 $("#mokName").val(mokName);
	 $("#mokCode").val(mokCode);
