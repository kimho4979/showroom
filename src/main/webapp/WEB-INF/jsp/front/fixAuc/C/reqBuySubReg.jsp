				...
				<div class="info_box">
					...
					<div class="m_col_wrap">
						<div class="m_col_left">
							<!-- 테이블03(S) -->
							<form id="frm" name="frm" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/front/fixCAuc/reqBuySubRegProc.do?fixSubDealSeq=${result.fixSubDealSeq}" >
									...
									<table>
										...
										<tbody>
													...
													<select id="jCode" name="jCode" onchange="getRecentList();">
														<c:forEach items="${floList}" var="floVO" varStatus="status">
															<c:if test="${floVO.bunChk eq 'C'}">
																<option value="${floVO.code}" <c:if test="${cFloLoginVO.chulCd eq  floVO.code}">selected="selected"</c:if>>${floVO.code} ( ${floVO.name} )</option>
															</c:if>
														</c:forEach>	
													</select>
													</div>
											...
											<tr>
												...
												<td class="tl">
													...
													<select id="chulCode" name="chulCode">
														<c:if test="${chulInfo ne null }">
															<option value="${chulInfo.code}" selected="selected">
															${fn:substring(chulInfo.code,0,4)} - ${fn:substring(chulInfo.code,4,8)} ( ${chulInfo.name} )
															</option>
														</c:if>
													</select>
													</div>
												...
												<td class="tl">
													<div class="sel_type_03 w50p">
														...
														<select id="mokCode" name="mokCode" onchange="getPumList();">
															<option value="<c:out value="${result.mokCode}"/>" selected="selected"><c:out value="${result.mokName}"/></option>
														</select>
											...
											<tr style="display: none;">
												...
												<td class="tl">
													<div class="sel_type_03 w50p">
														<input type="hidden" name="pumName" id="pumName" value="관엽류종합">
											...
											<tr>
												<th class="tc">품종명</th>
												<td class="tl"><p class="txt_01" id="pumNameCustom"><c:out value="${result.pumName}"/></p></td>
											</tr>
											<tr>
												<th class="tc">1상자당 분수량</th>
												<td class="tl"><p class="txt_01" id="pumNameCustom"><c:out value="${result.unitSok}"/></p></td>
											</tr>
											<tr>
												<th class="tc">상자수</th>
												<td class="tl">
													<div class="ip_type_01 w100">
														<input type="number" class="tr" id="boxCnt" name="boxCnt" maxlength="2" oninput="maxLengthCheck(this);fn_boxCntKeyUp('${result.unitSok}');">
											...
											<tr>
												...
												<td class="tl">
													<div class="ip_type_01 w100">
														<input type="number" class="tr" id="sokCnt" name="sokCnt" maxlength="4" oninput="maxLengthCheck(this);fn_sokCntKeyUp();">
											...
											<tr>
												...
												<td class="tl">
													<div class="ip_type_01 w100">
														<input type="number" class="tr" id="unitPrice" readonly="readonly" value="<c:out value="${result.unitPrice}"/>" name="unitPrice" maxlength="10" oninput="maxLengthCheck(this)">
							...
							<div class="btn_box mt30">
								...
								<a href="${pageContext.request.contextPath}/front/fixCAuc/fixBuySubList.do" class="btn_type_01 list">목록</a>
							</div>
						</div>
						<!-- 
...
function fn_boxCntKeyUp(unitSok){
	
	var boxCnt = $("#boxCnt").val();
	$("#sokCnt").val(boxCnt*unitSok);
	
}

function fn_sokCntKeyUp(){
	$("#boxCnt").val(1);
}
