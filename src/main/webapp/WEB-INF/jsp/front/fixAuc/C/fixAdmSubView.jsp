			...
			<div class="sub_conts_in">
				...
				<div class="info_box">
					...
					<div class="btn_box mt30">
						...
						<a href="${pageContext.request.contextPath}/front/fixCAuc/fixAdmSubList.do" class="btn_type_01 list">목록</a>
					</div>
					<!-- 버튼박스(E) -->
...
function fn_update(fixSubDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/fixAdmSubUpt.do?fixSubDealSeq="+fixSubDealSeq;
}

function fn_delete(fixSubDealSeq){
	if(confirm("삭제된 데이터는 복구 할 수 없습니다. 삭제하시겠습니까?")){
		window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/fixAdmSubDelProc.do?fixSubDealSeq="+fixSubDealSeq;	
	}
}

function fn_req(chulCode, mokCode, mokName, pumCode, pumName, unitPrice){
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/reqBuyReg.do?chulCode="+chulCode+"&mokCode="+mokCode+"&pumCode="+pumCode+"&unitPrice="+unitPrice+"&mokName="+encodeURI(mokName)+"&pumName="+encodeURI(pumName);
}
