...
function fn_update(fixDealSeq){
	window.location.href = "${pageContext.request.contextPath}/front/fixCAuc/fixAdmUpt.do?fixDealSeq="+fixDealSeq;
}
...
function fn_ban(fixDealSeq){
	...
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/fixAdmBan.json",
...
function fn_bidAdmin(bidSeq, fixDealSeq, bidAdminYn){
	$.ajax({
        ...
        type : "POST",
        url : "${pageContext.request.contextPath}/front/fixCAuc/fixAdmBidAdmin.json",
        success : function(data){
             alert("정삭적으로 처리되었습니다.");
             location.reload(true);
        }
	...
	if(bidCancelText != null && bidCancelText != ""){
		if(confirm("입찰 취소하시겠습니까?")){
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/fixCAuc/fixAdmBidCancel.json",
		        success : function(data){
		             alert("정삭적으로 처리되었습니다.");
		             location.reload(true);
		        }
...
function fn_bidUpdate(bidSeq, fixDealSeq){
	...
	if(confirm("입찰 수정하시겠습니까?")){
		$.ajax({
	        ...
	        type : "POST",
	        url : "${pageContext.request.contextPath}/front/fixCAuc/fixAdmBidUpdate.json",
	        success : function(data){
	             alert("정삭적으로 수정되었습니다.");
	             location.reload(true);
	        }
