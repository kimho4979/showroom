...
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
  ...
  <!-- sub-conts(S) -->
		<div class="sub_conts bgw">
			...
			<!-- sub상단(S) -->
			<div class="sub_top">
				<h2 class="sub_title">·출하자별 운송내역[집계]·</h2>
			...
			<div class="sub_conts_in">
				...
				<div class="info_box mt30">
					<div class="title_box">
						<div class="fl">
							<h4 class="sub_tit_02">출하자별 운송내역[집계]</h4>
						</div>
					</div>
					...
					<div class="title_box mt30">
						<div class="fl">
							<h4 class="sub_tit_04">출하자별 운송내역[집계] 검색</h4>
						</div>
					</div>
					<!-- 타이틀(E) -->
									...
									<div class="sb_data">
										<div class="table_devide mt10">
											<div class="date_from_to">
												<div class="date_box">
													<input type="text" id="panDate" name="panDate" class="datepicker" value="">
												</div>
											</div>
					...
					<div class="title_box mt30">
						<div class="fl">
							<h4 class="sub_tit_04">출하자별 운송내역[집계]</h4>
						</div>
						<div class="fr">
		...
		function fn_maxDate(){
			...
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/stat/transShprPartiTotSelectMaxPanDate.json",
		        success : function(data){
		        	var maxDate = "";
		        	if(data.maxVO != null){
		        		maxDate = data.maxVO.maxpanday;
		        		$("#panDate").val(maxDate);
		...
		function fn_search(){
			...
			$.ajax({
		        ...
		        type : "POST",
		        url : "${pageContext.request.contextPath}/front/stat/transShprPartiTot.json",
		        success : function(data){
		           var html = "";
		           var fhtml = "";
		           ...
		           if(resultList.length > 0){
		        	   for(var i=0; i<resultList.length; i++ ){
						...
						fhtml+="	<td class=\"tc\"><p class=\"txt_01\">"+comma(sumVO.chulcountsum)+"명</p></td>  ";
		...
		function fn_excell(){
			...
			var excellUrl = "${pageContext.request.contextPath}/front/stat/transShprPartiTotExcell.do?transCode="+transCode+"&panDate="+panDate+"&bunChk="+bunChk;
		...
		function fn_report(){
			var reportname = "transShprPartiTot.ozr";
			var transCode = $("#transCode").prop("value");
			var panDate = $("#panDate").val();
			var bunChk = $("input:radio[name=bunChk]:checked").val();
			
			$('input[name=reportname]').val(reportname);
			$('input[name=setUrl]').val("${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath}/front/stat/transShprPartiTot.json?transCode="+transCode+"&panDate="+panDate+"&bunChk="+bunChk);
