...
<script type="text/javascript">
	...
	function updateBoard(){
		...
		$.ajax({
			type: "POST",
			url: "${pageContext.request.contextPath}/admin/boardInfo/updateProc.json",
			data: $frm.serialize(),
	...
	function del(){
		...
		$.ajax({
   			url: "${pageContext.request.contextPath}/admin/boardInfo/deleteProc.json",
  	        type: 'POST',
  	        data:{
  	        	"boardId":boardId
  	        },
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<label class="checkbox-inline">
								<c:if test="${result.useThumbAt eq 'Y'}">
									<input type="checkbox" name="useThumbAt" id="useThumbAt" value="Y" checked="checked"/>
								</c:if>
								<c:if test="${result.useThumbAt ne 'Y'}">
									<input type="checkbox" name="useThumbAt" id="useThumbAt" value="Y"/>
								</c:if>
								리스트썸네일 사용
							</label>
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<div class="input-group">
								...
								<input type="text" id="uploadCount" name="uploadCount" value="${result.uploadCount}" class="form-control digits" />
							</div>
							<div class="input-group mg-t-xs">
								<span class="input-group-addon">사이즈</span>
								<input type="text" id="uploadSize" name="uploadSize" value="${result.uploadSize}" class="form-control digits" />
							</div>
							<div class="input-group mg-t-xs">
								<span class="input-group-addon">허용확장자</span>
								<input type="text" id="uploadExt" name="uploadExt" value="${result.uploadExt}" class="form-control digits" />
							</div>
						</div>
					</div>
					<div class="form-group">
